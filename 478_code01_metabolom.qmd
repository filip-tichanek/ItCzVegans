...
```{r}
summary(data_metabolites_original)
```
...
Metabolites that will show a significant diet effect (average effect of `diet` across both countries, adjusted for multiple comparisons with FDR < 0.1) will be then visualized using a forest plot, with country-specific diet effect along with diet effect based on independent validation cohort, to evaluate how generalizable are these findings. 
...
data.frame(
  ...
  beta = elanet_metabolit_all$betas[, 1]
) %>%
  arrange(abs(beta)) %>%
  filter(abs(beta) > 0,
...
kableExtra::kable(result_metabolom_val,
                  caption = 'Results of linear models estimating the effect of diet on metabolite levels. Only metabolites that significantly differed between diet groups in training cohorts (FDR < 0.1, average effect across both training cohorts) were included. `log2FD` represents the estimated effects (regression coefficient), indicating how much the log2-transformed metabolite levels differ between vegans and omnivores. `P`: p-value, `fdr`: p-value adjusted for multiple comparisons, and `CI_L` and `CI_U` represent the lower and upper bounds of the 95% confidence interval, respectively. All estimates in a single row are based on a single model.') 
